# AI Research Wiki Index

*Last updated: 2026-06-01*

A structured knowledge base for tracking state-of-the-art AI techniques, papers, and research developments.

---

## Techniques (12)
*Core AI methods and approaches*

- [RLHF](technique/reinforcement-learning-from-human-feedback.md) — Trains AI systems using human preferences via RL | active | 2026-05-25
- [Managed Agents](technique/managed-agents.md) — Decoupled agent architecture for reliability and scale | active | 2026-05-26
- [Long-Running Agent Harnesses](technique/long-running-agent-harnesses.md) — Dual-agent workflow patterns for multi-session continuity | superseded | 2026-05-26
- [GAN-Inspired Agent Architectures](technique/gan-inspired-agent-architectures.md) — Generator-evaluator patterns for iterative quality improvement | active | 2026-05-26
- [Session-Based Context](technique/session-based-context.md) — External context storage via durable event logs | active | 2026-05-25
- [Attribution Graphs](technique/attribution-graphs.md) — Circuit tracing for mechanistic interpretability of LLMs | active | 2026-05-25
- [Cross-Layer Transcoders](technique/cross-layer-transcoders.md) — Linear feature interactions enabling attribution graphs | active | 2026-05-25
- [Sparse Autoencoders](technique/sparse-autoencoders.md) — Dictionary learning for interpretable feature discovery | active | 2026-05-25
- [Circuit Tracing](technique/circuit-tracing.md) — Maps computational pathways in neural networks | active | 2026-05-25
- [Mechanistic Interpretability](technique/mechanistic-interpretability.md) — Reverse-engineering neural network algorithms | active | 2026-05-25
- [Test-Time Compute](technique/test-time-compute.md) — Scales inference compute for better reasoning performance | active | 2026-06-01
- [Defensive Acceleration](technique/defensive-acceleration.md) — Structured deployment ensuring defenders get preferential access | active | 2026-06-01

---

## Sources (9)
*Papers, blog posts, talks, and other materials*

- [InstructGPT](source/instructgpt-2022.md) — Training language models to follow instructions | paper | 2022-03-04
- [Managed Agents Blog](source/anthropic-managed-agents-2026.md) — Decoupling the brain from the hands | blog | 2026-05-25
- [Biology of a Large Language Model](source/anthropic-biology-llm-2025.md) — Mechanistic interpretability of Claude 3.5 Haiku | paper | 2025-01-14
- [Tracing the Thoughts](source/anthropic-tracing-thoughts-2025.md) — Attribution graphs reveal Claude's internal mechanisms | blog | 2025-01-14
- [Circuit Tracing Methods](source/anthropic-circuit-tracing-2025.md) — Cross-layer transcoders for attribution graph construction | paper | 2025-01-14
- [Long-Running Agent Harnesses](source/anthropic-long-running-agents-2025.md) — Dual-agent workflows for multi-session continuity | blog | 2025-11-26
- [GAN-Inspired Harness Design](source/anthropic-harness-design-2026.md) — Multi-agent architectures with generator-evaluator patterns | blog | 2026-05-26
- [OpenAI Discrete Geometry](source/openai-discrete-geometry-2026.md) — AI model disproves 80-year-old mathematical conjecture | blog | 2026-05-29
- [OpenAI Rosalind Biodefense](source/openai-rosalind-biodefense-2026.md) — Defensive acceleration strategy for biological capabilities | blog | 2026-05-20

---

## Benchmarks (0)
*Evaluation suites and datasets*

*(No entries yet)*

---

## Concepts (5)
*Definitional pages for important terms*

- [Alignment](concept/alignment.md) — Problem of ensuring AI systems pursue intended goals | 2026-05-25
- [Context Anxiety](concept/context-anxiety.md) — Premature task termination near context limits | 2026-05-25
- [Multi-Step Reasoning](concept/multi-step-reasoning.md) — Internal reasoning steps in language models | 2026-05-25
- [Planning in LLMs](concept/planning-in-llms.md) — Forward and backward planning capabilities | 2026-05-25
- [Metacognition](concept/metacognition.md) — Self-awareness of knowledge and reasoning | 2026-05-25

---

## Threads (1)
*Evolving research narratives*

- [Frontier Research Capabilities](thread/frontier-research-capabilities.md) — AI systems contributing to cutting-edge scientific research | active | 2026-06-01

---

## Questions (1)
*Open research questions to track*

- [RL Stage Necessity](question/rl-stage-necessity.md) — Can we achieve RLHF benefits without RL? | open | 2026-05-25

---

## Recent Activity

See [log.md](log.md) for detailed history.

## Quick Navigation

- [Inbox](_inbox.md) — Techniques mentioned but not yet covered
- [Log](log.md) — Activity history
- [Schema](../AGENTS.md) — Instructions and conventions